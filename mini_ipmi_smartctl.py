...
rellocatedSectors = (
    '^(?:\s+)?\d+\s+Reallocated_Sector_Ct\s+[\w-]+\s+\d{3}\s+[\w-]+\s+[\w-]+\s+[\w-]+\s+[\w-]+\s+[\w-]+\s+(\d+)',
#    '^(?:\s+)?\d+\s+Reallocated_Event_Count\s+[\w-]+\s+\d{3}\s+[\w-]+\s+[\w-]+\s+[\w-]+\s+[\w-]+\s+[\w-]+\s+(\d+)',
    '^(?:\s+)?Rellocate:\s+(\d+)\s+C',
)
...
def findDiskRellocated(p):
    resultA = None
    for i in rellocatedSectors:
         rellocatedSe = re.search(i, p, re.I | re.M)
         if rellocatedSe:
             resultA = rellocatedSe.group(1)
             break
    #resultA = 1
    return resultA
...
def isNVME(p):
    ...
    nvmeRe = re.search('^NVMe Version:', p, re.I | re.M)
...
if __name__ == '__main__':
    ...
    for d in diskDevs:
        ...
        sector = findDiskRellocated(diskPout)
        if isDuplicate:
        ...
        if     (temp and
            ...
            elif isNVME(diskPout):
                threshMin  = thresholds[2][1]
                threshMax  = thresholds[2][2]
                threshCrit = thresholds[2][3]
            else:
        ...
        if sector:
            senderData.append('"%s" mini.disk.rellocated[%s] "%s"' % (HOST, sanitizedD, sector))
